...
## Unreleased — flexible integration-workflow + general semantic-merge refactor

- **Phase B flattened to a review-led loop; Tier matrix removed.** `integration-workflow` Phase B is rewritten as a four-step loop: (1) integration reviewer dispatched — walks branch diff and main-side diff, writes `## Integration Intent` in PLAN.md when material incoming changes found, annotates per-task blockquotes, flips REVISE; (2) orchestrator adjudicates and batches any user decision via `AskUserQuestion`; (3) fix-review loop — mechanical merge first (semantic-merge when the reviewer calls for intent-based resolution, not a Tier gate), then refactor commits scoped to REVISE tasks, then reviewer re-review until APPROVE; (4) orchestrator flips `Refactored` milestone when all tasks APPROVED and Integration Intent empty/absent. Tier 1/2/3 matrix, two shortcut axes, recon/verify reviewer naming split, two-commit implementer contract, and delegated mode all removed. No behavioral loss — the same patterns express as reviewer annotations rather than as a classification gate.
- **`semantic-merge` generalized for any vertical and any caller.** `skills/semantic-merge/SKILL.md` rewritten on top of the global `semantic-merge-integration` baseline. Standalone/Delegated mode split, named return-field contract, in-skill dispatch blocks, and Tier 1/2/3 matrix removed. The skill now reads coherently for a human at the terminal, an orchestrator running it directly, or a dispatched agent. The 1+N commit shape is stated as one possible workflow, not mandated. Domain-neutral vocabulary throughout (no analysis-specific examples in general criteria text).
- **`## Integration Intent` section added to PLAN.md anatomy.** `skills/handoff-doc/references/plan-anatomy.md` gains a new `## Integration Intent` section: purpose, ownership (integration reviewer only; implementer hands-off), lifecycle (write on Phase B Step 1 scan → per-item removal when the last named task reaches APPROVED → section removal when empty), and format. Placement: after `## Decisions` if present, before the first task block.
- **Parallel-reviewer pattern added to `agent-orchestration`.** `skills/agent-orchestration/SKILL.md` §Concurrent Writers Require Worktree Isolation extended with a paragraph covering parallel reviewers walking disjoint slices of a large diff — same worktree isolation, same disjoint-scope invariant, same orchestrator-aggregates-verdicts pattern. `Worktree:` dispatch field now applies to reviewers in this configuration.

## Unreleased — unified integration-workflow refactor

- **Integration-status flips are reviewer/implementer-owned.** Correcting an error in the first cut of Task 8: the recon reviewer flips annotated tasks to `Integration status: REVISE` in its own review commit (symmetric with `Review status: REVISE`); the unified implementer flips in-scope tasks to `IMPLEMENTED` at Commit 2; the verify reviewer flips to `APPROVED` (or back to `REVISE` on failing tasks) in its review commit. The orchestrator does not flip by default; it only overrules via `→ orchestrator:` annotations. Touches `skills/handoff-doc/references/plan-anatomy.md` (state-setter clause rewritten), `skills/integration-workflow/SKILL.md` (Phase B Step 2 retitled "evaluate shortcuts"; Steps 3 and 4 rewritten to name each actor), `agents/reviewer.md` and `agents/implementer.md` (§What You Own extended to cover `**Integration status:**`).
- **`merge-workflow` removed.** The former `skills/merge-workflow/` is deleted; its choreography (main update via `semantic-merge`, post-merge verification, local merge or PR push, worktree cleanup) is folded into `superRA:integration-workflow` Phase D. Callers across `skills/`, `agents/`, `README.md`, `CLAUDE.md`, `skills/CATEGORIES.md`, and `tests/structural-invariants.sh` repointed at `integration-workflow`. `semantic-merge` remains a standalone utility, invoked from `integration-workflow` Phase B (recon + Tier 2/3 implementer) and usable standalone.
- **Peripheral-surface sync.** `skills/using-superRA/SKILL.md` skill inventory drops the `merge-workflow` row and clarifies that the Manifest's `merge` stage is for standalone `semantic-merge` delegated-mode dispatches only (inside Phase B, the unified implementer runs `Stage: integration` and conditionally loads `semantic-merge` via the canonical `Skills:` field on Tier 2/3). `skills/execution-workflow/SKILL.md` Step 4 completion menu (Options 1/2) hands off to `integration-workflow` alone. `skills/handoff-doc/references/plan-anatomy.md` Workflow Status milestones renamed to Phases A–D; `Merged` milestone points at Phase D. `skills/agent-orchestration/SKILL.md` reviewer-feedback-discipline call-site list updated. `README.md` skill inventory + Workflow Map Mermaid diagram updated (Phase D node replaces MERGE; B→B re-entry + D→B re-entry arrows added). `skills/CATEGORIES.md` row for `merge-workflow` removed; `integration-workflow` row widened to Phases A–D. `/CLAUDE.md` §Workflow principles workflow-skills list updated.
...
- **Per-task `**Integration status:**` field** added to the task-block template in `handoff-doc/references/plan-anatomy.md` alongside `**Review status:**`. Values: unset / IMPLEMENTED / REVISE / APPROVED. Reviewer/implementer-owned: the recon reviewer flips to `REVISE` on annotated tasks, the unified implementer flips to `IMPLEMENTED` at Phase B Commit 2, the verify reviewer flips to `APPROVED` (or back to `REVISE` on failing tasks) — symmetric with `**Review status:**`; the orchestrator only overrules via `→ orchestrator:` annotations.
- **`## Workflow Status` project-level boxes are a rollup over per-task states.** The orchestrator unchecks boxes at re-entry by judgment — not by formula — and declares in a §Decisions entry which boxes and why. Tasks fully re-implemented have both `**Review status:**` and `**Integration status:**` cleared; tasks untouched retain APPROVED; minor-edited tasks may clear only `**Integration status:**`.
- **DAG transitive-downstream cascade.** On re-entry, the orchestrator walks the `**Depends on:**` DAG and clears `**Review status:**` and `**Integration status:**` for all tasks in the transitive downstream closure of any task whose code or outputs change. Exemptions require a documented blockquote per exempted task in §Decisions.
- **Full drift-test suite always runs.** Only *authoring* new drift tests is scoped to relevant tasks; running is not. `integration-workflow` Stage 1 always dispatches the full suite regardless of which tasks were cleared. Doc-writer always re-runs the whole doc; doc-reviewer reviews the diff.
- **Banned shortcut added.** Running a subset of the drift-test suite on re-entry because "only these tasks changed" is now an explicit banned shortcut in the section.

**`plan-anatomy.md` updates.** `**Integration status:**` appears in the task-block template, in the `## Workflow Status` header paragraph (with rollup semantics and re-entry unchecks), and in the Field-by-Field section with the same cascade rule as `**Review status:**`. The `## Workflow Status checkboxes` Field-by-Field bullet pointer retargeted from `handoff-doc §Mid-Session Scope Changes` to `planning-workflow §Changing Plans`.

**Main-agent reference consolidation.** `skills/using-superRA/references/session-bootstrap.md` and `skills/using-superRA/references/main-agent-autonomy.md` merged into a single `skills/using-superRA/references/main-agent.md`. The consolidated reference carries: MANDATORY Session Start Actions, Cross-Session Detection (bash block + incomplete-plan handling + worktree handling), a new **Load the Handoff-Doc Skill** section, The Three Pause Classes, Proceed Without Asking, Banned Phrasings, One Question at a Time, and Log Before You Act. Main agents now load `superRA:handoff-doc` by default at session start so editing discipline and `§Scope Changes and Re-entry` are available before touching PLAN.md — no separate skill invocation required. `skills/using-superRA/SKILL.md` Skill-Load Manifest preamble updated with a "Main-agent default load" note; the subagent-side manifest rows are unchanged.

**Wire-ups across workflow skills.** `execution-workflow` Step 1 sub-step 2a is rewritten as resumption-of-paused-protocol, reading per-task `**Review status:**` and `**Integration status:**` fields on re-entry and resuming `planning-workflow §Changing Plans` at Step 6 when a prior invocation has paused; its Stop-Points pointer retargets to `planning-workflow §Changing Plans`. `integration-workflow` Stage 1 adds the always-full-drift-suite rule and updates the `Drift tests created` box-flip to key on all tasks having `**Integration status:** APPROVED`; Stage 2 adds a refactorer scope clause and the same rollup for `Refactored`; Step 3 adds the doc-writer-always-full and doc-reviewer-diff rules with the `Docs finalized` box-flip updated to rollup. `merge-workflow` carries no prior pointer and needs no change. Stale pointers in `agent-orchestration/references/agent-teams.md`, `skills/CATEGORIES.md`, and both agent files' autonomy references updated to `main-agent.md`.

**Files touched.** `skills/planning-workflow/SKILL.md` (new `## Changing Plans` section — 6-step protocol + DAG cascade + banned shortcuts); `skills/handoff-doc/SKILL.md` (prior `§Scope Changes and Re-entry` body removed; one-sentence pointer retained under `§PLAN.md Is the Task Tracker`); `skills/handoff-doc/references/plan-anatomy.md` (Integration status field + Workflow Status description + Field-by-Field cascade; pointer retargeted to `planning-workflow §Changing Plans`); `skills/execution-workflow/SKILL.md` (Step 1 sub-step 2a rewritten as resumption-of-paused-protocol; Stop-Points pointer retargeted to `planning-workflow §Changing Plans`); `skills/integration-workflow/SKILL.md` (always-full-drift-suite + refactor scope + doc-writer/reviewer scope + box-flip rollup language; pointers retargeted); `skills/using-superRA/SKILL.md` (main-agent default-load paragraph; **Significant plan changes re-enter planning-workflow** paragraph); `skills/using-superRA/references/main-agent.md` (new consolidated reference); `skills/using-superRA/references/session-bootstrap.md` (deleted); `skills/using-superRA/references/main-agent-autonomy.md` (deleted); `agents/implementer.md` + `agents/reviewer.md` (header-ownership bullet in "You may NOT edit" list; autonomy pointer updated); `CLAUDE.md` + `README.md` (iterative-workflow reframing); `skills/agent-orchestration/references/agent-teams.md` + `skills/CATEGORIES.md` (stale pointers retargeted to `main-agent.md`).

## 2026-04-17 — Agent dispatch refactor (feedback round)

- **Parallel implementers in isolated worktrees.** `agent-orchestration` gains §Concurrent Writers Require Worktree Isolation: parallel dispatches of ≥2 implementers now require one worktree per implementer on a `parallel/<analysis-branch>/<slug>` branch, seeded with force-symlink non-git data, merged back with plain `git merge --no-ff`. Task boundaries are set ex-ante in `PLAN.md` so merges are mechanically disjoint — `semantic-merge` is bypassed for these, and `hooks/merge-guard` exempts `parallel/*` source refs. Dispatch template gains optional `Worktree:` field + required canned steering sentence (the single exception to the additive-only `Additionally:` rule).
- **`RESULTS.md` pre-allocated at planning time.** One `## Task N: <name>` stub per PLAN.md task block, body `**Status:** Not started`. Implementer replaces its stub's content in place on first pass; never appends a new section at EOF. Pre-allocation matches PLAN.md's task-block layout and keeps parallel edits as disjoint mid-file hunks so `git merge` auto-resolves. Updated `handoff-doc/references/results-anatomy.md`, `planning-workflow/SKILL.md`, and `agents/implementer.md`.
- **`using-superRA` §Shared-Repo Commit Discipline renamed to §Commit Hygiene.** Worktree isolation handles the multi-writer case the old framing was defending against; the remaining motivation is defensive hygiene against untracked editor / harness / cache artifacts (`.claude/settings.local.json`, `__pycache__`, `.DS_Store`) that `git add -A` would silently sweep into a commit. Same mechanics (stage by exact path, `git diff --cached` before commit); simpler justification. `agents/implementer.md` collapses the path-A/path-B commit-discipline split into a single default; the `Worktree:` field only alters the no-merge / branch-reporting aspects. `agents/reviewer.md` updated pointer.
- **`worktree-data-sync` scope narrowed** to non-git data sync only (seed, diff, apply, data teardown). Worktree lifecycle (create / enter / remove) moved to `skills/agent-orchestration/references/worktree-harness-fallback.md` — harness tools preferred, raw `git worktree` otherwise. Consumer workflows (`planning-workflow`, `execution-workflow`, `merge-workflow`, `semantic-merge`, plus references under `using-superRA` and `agent-orchestration`) updated with pointer-only edits.
- `agent-orchestration`: Agent Teams mode archived (unreliable in practice). `references/agent-teams.md` carries an ARCHIVED banner; no active skill, agent, or hook loads or cites it. Replaced with §Workload Balancing: three-tier dispatch framework (trivial→inline, slightly-involved→bundle-and-delegate, complicated→dedicated agent), ≤150k tokens/agent rule of thumb, cache-reuse guidance. Decision Framework DOT graph + pattern table removed. §Agent reuse vs fresh dispatch removed. Dispatch template `<optional steering>` tightened to additive-only (never restate defaults, manifest, or PLAN.md content).
- `using-superRA`: new §Shared-Repo Commit Discipline — binds every agent (main/orchestrator + subagents) that stages a commit when other agents may be writing in the same repo; teaches path-specific staging (`git add <path>`), pre-commit diff inspection, and escalation when unfamiliar changes appear in `git status`. `agents/implementer` + `agents/reviewer` point at this section.
- `planning-workflow`: plans now declare `**Depends on:**` per task block (lightweight DAG, not a DSL); `execution-workflow` reads these fields before dispatching to batch independent tasks in parallel.
- `handoff-doc`: User Decisions Log "Not covered here" negation block removed from `references/plan-anatomy.md` — the positive-inclusion rule carries the discipline on its own.
...
**Workflow-stage markers + mid-session scope-change protocol in `PLAN.md`.** Every `PLAN.md` header now carries a `## Workflow Status` checklist of six irreversible milestones (`Plan approved`, `Execution complete`, `Drift tests created`, `Refactored`, `Docs finalized`, `Merged`) that a resuming agent reads to know which workflow stage this branch is at — no grep-the-commit-log needed. Each box is flipped by the workflow skill that owns the corresponding gate: `planning-workflow` Execution Handoff flips `Plan approved`; `execution-workflow` Step 3 flips `Execution complete`; `integration-workflow` flips `Drift tests created` / `Refactored` / `Docs finalized` at the three APPROVE points; `merge-workflow` Step 4 flips `Merged` (and Step 3 unchecks `Refactored` on post-merge re-entry). Subagents never flip boxes — the header is orchestrator-owned, codified in new bullets in `agents/implementer.md` and `agents/reviewer.md`. `handoff-doc/SKILL.md` gains two new top-level sections: **§PLAN.md Is the Task Tracker** (establishes `PLAN.md` as the persistent tracker, `TodoWrite` as a transient session view), and **§Mid-Session Scope Changes** (6-step confirm → log → inline-edit → roll-back-milestones → atomic-commit → resume protocol for researcher-initiated scope changes mid-execution). `planning-workflow` Living Plan section distinguishes agent-discovered drift (inline edit) from researcher-initiated scope changes (routed to the new protocol); `execution-workflow` stop-points class (b) points at the same protocol. `handoff-doc/references/plan-anatomy.md` carries the `## Workflow Status` block inside the header template, extended Header-ownership note naming the three orchestrator-owned regions (header proper, Workflow Status, Decisions), and a Field-by-Field bullet spelling out box-flip mechanics.
