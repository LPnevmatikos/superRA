...
>    → implemented: rewrote the cache-reuse rule of thumb to clarify that `SendMessage` is a standard harness feature (not Teams-specific) available when `Agent` is used with `run_in_background: true` and a background agent is still live; preserved the valid TTL observation; added bundling as an alternative path for amortizing spawn cost. (`skills/agent-orchestration/SKILL.md` lines 65–77)
