...
> → implemented: per follow-up user feedback on the prior revision (still messy, scope-change edge missing arrowhead, `§User Feedback and Changing Plans` label unwanted), rewrote the diagram from scratch along the simpler-alternative path. Dropped both subgraphs entirely — PLAN, IMPLEMENT, INTEGRATE are now three flat nodes with the per-task implementer ⇄ reviewer loop and the A → B → C → D chain collapsed into each node's multi-line label. Scope-change re-entry is a single directed dotted edge `INTEGRATE -. "scope change" .-> PLAN` (arrowhead via `.->`, label trimmed to "scope change" with no cross-reference). Rendered to `/tmp/wfdiag/v2.png` at `-s 2` and inspected via Read: three phase boxes sit in a single vertical column; the scope-change dotted arrow curves out the right side of INTEGRATE, up the right margin, and lands on PLAN with a visible arrowhead; no edge crosses through the column; no `§User Feedback and Changing Plans` text appears.
...
> → implemented: re-embedded the rewritten diagram — replaced only the ```mermaid ... ``` fenced block in README §Plan-Implement-Integrate Workflow with the body of the revised `docs/drafts/workflow-diagram.mmd` (three flat phase nodes, side-routed arrowheaded scope-change edge; see Task 1a annotation). All surrounding prose (the paragraph above the diagram, §Key principles, §Domain Skills, §Utility Skills, §Installation, §Hooks, §Contributing, §Upstream, §License) stays byte-identical to the prior APPROVED state.
