...
- **Phase B** — *unified* sync + refactor. Internal structure: recon reviewer (writes per-task annotations + flips `Integration status: REVISE` on annotated tasks in its own review commit) → orchestrator evaluates shortcuts → optional batched user decisions → unified implementer (two-commit structure; flips `Integration status: IMPLEMENTED` on each in-scope task at Commit 2) → verify reviewer (flips `Integration status: APPROVED` on APPROVE, or back to `REVISE` on specific failing tasks). Integration-status flips are reviewer/implementer-owned, symmetric with `Review status` — the orchestrator does not flip by default, only overrules via `→ orchestrator:` annotations. Iterative: re-entrant on itself (`B → B`) when main advances again before Phase D.
...
- **Integration-status lifecycle is reviewer/implementer-owned, symmetric with Review status.** The `plan-anatomy.md` state-setter clause for `**Integration status:**` names the full choreography — the recon reviewer flips annotated tasks to `REVISE` in its own review commit, the unified implementer flips in-scope tasks to `IMPLEMENTED` when it commits Commit 2, and the verify reviewer flips to `APPROVED` (or back to `REVISE` on failing tasks) in its review commit. The orchestrator does not flip by default; it only overrules via `→ orchestrator:` annotations, same as for Review status. `integration-workflow` Phase B Step 2 (retitled "Orchestrator — evaluate shortcuts", no longer "flip statuses"), Step 3 (Commit 2 handoff), and Step 4 (APPROVE/REVISE bullets) were updated to name the actor on every flip. `agents/reviewer.md` §What You Own and `agents/implementer.md` §What You Own were extended to cover the `**Integration status:**` line. Replaces an earlier attempt where the orchestrator was the one flipping `REVISE` post-recon and `APPROVED` post-verify — that design broke the reviewer-owns-its-verdict principle that already governed Review status.
